...
- [ ] (v3.0 revision) Confirm via Supabase MCP that "The Corporate Live Build"
      (`qdqvpctmotquybvtzwzs`) is still empty, then create the `suppliers` and
      `submissions` tables and their anon insert-only RLS policies.
- [ ] (v3.0 revision) Write `docs/supabase-setup.md` documenting the project
      (this is the first tool to formally document its schema).
- [ ] (v3.0 revision) Build the Supplier Gate view (company name, contact name,
      contact email, EcoVadis Yes/No, GDPR consent) and wire it as the Landing
      page's single CTA, ahead of Door Selection.
- [ ] (v3.0 revision) Update the Landing page: replace the old two-button "Two
      Routes" action area with the single CTA into the Supplier Gate.
- [ ] (v3.0 revision) Wire `suppliers.id` from the Gate insert through Door
      Selection and whichever door is used, so the `submissions` insert links
      via `supplier_id`.
- [ ] (v3.0 revision) Add the GDPR consent checkbox and confirmed data statement
      to the Gate.
- [ ] (v3.0 revision) Test locally, including verifying inserts against the live
      Supabase project, before deploying.
- [ ] (v3.0 revision) Acceptance criteria pass — verify all 17 criteria in
      product-spec.md v3.0 Section 13 before deploy. (Supersedes the earlier
      "criterion 16" item above, which referred to v2.0's numbering.)
- [ ] (v3.0 revision) Deploy to Netlify and add the Supabase anon key as a
      Netlify environment variable; confirm Supabase inserts succeed from the
      deployed site, not just locally.
- [ ] (v3.0 revision) Optional post-build: run the Supabase QA skill to verify
      the schema and RLS policies.
...
- Spec revised to v3.0 on 12 July 2026 — CLAUDE.md regenerated by Project Governor.
- product-spec.md v3.0 (Section 4 and Acceptance Criterion 16) describes the
  Supabase project as already sharing a schema with the GHG emissions dashboard;
  as confirmed by the builder, the project is actually still empty and not yet
  shared with any other tool. CLAUDE.md and this file reflect the real,
  empty-project state — the spec text should be corrected on its next revision.
